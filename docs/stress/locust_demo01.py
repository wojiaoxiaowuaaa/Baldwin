...
class WebsiteTask(TaskSet):
    ...
    def __login(self):
        ...
        res = self.client.post(
            login_api, login_data, headers=login_header, cookies=cookie
        )
        print("====== 登录成功 ======")
    ...
    def api_get_member(self):
        # 获取项目空间成员接口
        role_data = {"projectId": 100404, "pageStart": 1, "page": 1, "pageSize": 10}
        res = self.client.post(
            "/jbdpEdc/api/getRolePage", json.dumps(role_data), headers=self.api_header
        )
        # print(res.json())
...
if __name__ == "__main__":
    ...
    os.system(
        "locust -f locust_demo01.py --headless -u 10 -r 1 -t 10 --html report.html"
    )
