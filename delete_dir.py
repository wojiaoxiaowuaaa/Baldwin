...
def clean_dir(dir_path: str) -> None:
    ...
    if os.path.exists(dir_path):
        try:
            ...
            logging.info(f"Directory '{dir_path}' deleted successfully.")
        except PermissionError:
            logging.error(f"Permission denied: Unable to delete '{dir_path}'.")
        except Exception as e:
            logging.error(f"Error during clean up: {e}")
    else:
...
def main(args: Optional[list[str]] = None) -> None:
    ...
    if args:
        ...
        print("Usage: python clean_dir.py <absolute_directory_path>")
