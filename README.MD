...
d = {'a': 24, 'g': 52, 'i': 12, 'k': 33} # print(sorted(d.items(), key=lambda x: x[1])) # items() 方法是 Python 字典对象的一个内置方法，它返回一个包含字典键值对的视图对象。这个视图对象是一个可迭代的对象，其中每个元素都是一个包含键值对的元组 (key, value)。
