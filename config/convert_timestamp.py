...
def timestamp_to_beijing_time(timestamp):
    if len(str(timestamp)) == 13: timestamp /= 1000  # Convert milliseconds to seconds if it's in milliseconds

    utc_time = datetime.fromtimestamp(timestamp, tz=timezone.utc)
    beijing_time = utc_time.astimezone(timezone(timedelta(hours=8)))  # Convert to Beijing timezone
    return beijing_time
