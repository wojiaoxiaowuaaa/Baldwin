...
# 用于确保在任何给定时刻只有一个线程在解释器中执行Python字节码，即在单个进程内，Python
# 解释器的并发性被限制在一个线程上执行。这意味着在多线程程序中，多个线程不能同时利用多核处理器的能力。
# GIL的存在主要是为了简化Python解释器的实现，以避免在解释器内部处理线程间的资源竞争和复杂性。
# 然而，它也导致了Python在CPU密集型任务上的性能限制。对于I/O密集型任务，GIL通常不会成为性能瓶颈，
# 因为线程在等待I/O操作时会释放GIL.下面是一个简单的示例来说明GIL的影响.考虑一个计算密集型任务计算一个大素数列表的示例：
